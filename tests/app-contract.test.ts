...
test('merchant feedback center provides product applications and feedback flows', () => {
  ...
  assert.match(appSource, /暂无可申请商品，请切换到“申请新增商品”。/)
...
test('merchant existing product application uses compact product picker with selected card', () => {
  ...
  assert.match(appSource, /没有找到商品，请切换到申请新增商品/)
  ...
  assert.match(cssSource, /\.product-application-mobile-sheet\s*\{[\s\S]*grid-template-rows:\s*auto minmax\(0,\s*1fr\)[\s\S]*overflow:\s*hidden/)
