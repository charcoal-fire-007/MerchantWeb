...
test('machine inventory page supports compact batch snapshot submission', () => {
  ...
  assert.match(appSource, /选择机器/)
  assert.match(appSource, /上报清单/)
  assert.match(appSource, /库存数量/)
  assert.match(appSource, /我的商品/)
  assert.match(appSource, /平台机器库/)
  assert.match(appSource, /平台库/)
  assert.match(appSource, /待申请/)
  assert.match(appSource, /inputmode="numeric"[\s\S]*min="0"[\s\S]*step="1"/)
  ...
  assert.match(appSource, /class="inventory-submit-footer"/)
...
test('mobile layout moves primary navigation into a bottom tab bar', () => {
  ...
  assert.match(appSource, /navActiveIndex = computed\(\(\) => \(\{ dashboard: 0, products: 1, machineInventory: 2, notifications: 3, feedback: 4 \}\[navActive\.value\] \?\? 0\)\)/)
  ...
  assert.match(cssSource, /@media\s*\(max-width:\s*768px\)[\s\S]*\.sidebar-nav\s*\{[\s\S]*grid-template-columns:\s*repeat\(5,\s*minmax\(0,\s*1fr\)\)/)
...
test('merchant feedback center provides product applications and feedback flows', () => {
  ...
  assert.match(appSource, /@click="applyProductApplicationSearch"/)
  ...
  assert.match(cssSource, /\.feedback-record-actions\s*\{/)
  assert.match(cssSource, /\.feedback-record-toggle\s*\{/)
})
...
test('merchant existing product application uses compact product picker with selected card', () => {
  ...
  assert.match(appSource, /function toggleProductApplicationPicker\(\)/)
  ...
  assert.match(appSource, /@mousedown\.prevent="toggleProductApplicationPicker"/)
  assert.match(appSource, /class="product-application-mobile-sheet"/)
  assert.match(appSource, /没有找到「/)
  assert.match(appSource, /@mousedown\.prevent="chooseNewProductApplicationFromSearch"/)
...
test('merchant price feedback uses a custom product picker instead of native datalist', () => {
  ...
  assert.match(cssSource, /\.feedback-product-mobile-sheet\s*\{[\s\S]*grid-template-rows:\s*auto minmax\(0,\s*1fr\)[\s\S]*overflow:\s*hidden/)
